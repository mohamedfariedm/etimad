        ...
        {["fast", "secure", "easy"].map((item, index) => (
          <div key={index} className="w-[368px] h-[101px] shrink-0 relative z-[7]">
            ...
            <div
              ...
              style={{ backgroundImage: `url(/assets/images/eassyway/frame${index + 1}.svg)` }}
            />
            <div className="flex w-[212px] h-[61px] flex-col gap-[4px] items-start flex-wrap xl:flex-nowrap absolute top-1/2 left-[68px] translate-x-0 translate-y-[-50%] z-[9]">
              <span className="h-[30px] self-stretch shrink-0 basis-auto text-[20px] font-medium leading-[30px] text-[#5d9d9f] relative text-right xl:whitespace-nowrap z-10">
                {t(`easyWay.${item}.title`)}
              </span>
              <span className="flex w-[218px] h-[27px] justify-start items-start shrink-0 basis-auto text-[14px] font-normal leading-[26.6px] text-[#5d9d9f] relative text-right xl:whitespace-nowrap z-[11]">
                {t(`easyWay.${item}.description`)}
              </span>
            </div>
          </div>
        ))}
